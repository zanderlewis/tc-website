...
def asheville():
    ...
    for category in categories:
        try:
            ...
            debug(
                f"Found {len(menu_items)} items in category {category_html.find('h5').text}"
            )
            ...
            for item in menu_items:
                ...
                description_element = (
                    item.find("p")
                    if item.find("p") and item.find("p").text.strip() != ""
                    else ""
                )
                ...
                if (
                    ...
                    c.execute(
                        "INSERT INTO ashevillenc VALUES (?, ?, ?, ?)",
                        (name, description, price, section),
                    )
                else:
                    debug(
                        f"Skipping item due to missing data: {item}: {name_element}, {description_element}, {price_element}, {section_element}"
                    )
